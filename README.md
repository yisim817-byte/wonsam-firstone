...
## Production SEO 기준선 (2026-08-21)

- 프로덕션 대표번호는 `1833-3872`입니다.
- canonical·sitemap·robots.txt·manifest·JSON-LD·URL 정규화 기준은 2026-08-21 검증본을 기준으로 유지합니다.
- SEO 변경은 프리뷰 렌더링, 구조화 데이터 JSON 파싱, canonical·sitemap 정합성 확인 후 프로덕션에 반영합니다.
...
- 일반고객: `index.html`/`consultation.html`에서 전화상담(`tel:18333872`) 중심으로 안내합니다. 폼을 제공하지 않습니다.
...
- 공개 연락처는 대표번호 `1833-3872`만 표시합니다. 이름, 휴대폰 번호 등 개인정보성 연락처는 공개 화면에 올리지 않습니다.
...
- 목업의 가짜 연락처("16XX-XXXX")는 실제 현장 안내 대표번호(1833-3872)로 교체했습니다.
...
- **전화번호 강조 컴포넌트** (`.phone-cta`): 대표번호(1833-3872)를 26px 굵은 글씨로 크게 표시하고 `tel:18333872` 링크를 걸었습니다. 정적 사이트지만 `contact.js`를 모든 HTML에 1회씩 포함해 전화 링크와 화면 표시 번호를 대표번호 기준으로 보정합니다.
...
- FormSubmit, `contact.js`, 대표번호 `1833-3872`/`tel:18333872`은 전혀 변경하지 않았습니다.
