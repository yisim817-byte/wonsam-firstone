...
- 입력한 비밀번호는 API 호출 시 `Authorization: Bearer <token>`으로 전달된다.
- 서버 API는 Vercel 환경변수 `ADMIN_TOKEN`과 비교한다.
- 토큰이 없거나 틀리면 접수 데이터는 반환하지 않고 401을 반환한다.
...
- 관리자 올바른 비밀번호 입력 후 3종 접수 목록 표시 확인
